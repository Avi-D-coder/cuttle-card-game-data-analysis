...
from .material import apply_material_scores
from .metrics import (
...
def main() -> None:
    ...
    if out_dir:
        win_table.to_csv(out_dir / "win_rates_by_diff.csv", index=False)
        flip_table.to_csv(out_dir / "flip_rates_by_diff.csv", index=False)
        deck_table.to_csv(out_dir / "deck_depth_stats.csv", index=False)
        win_deck_table.to_csv(out_dir / "win_rates_by_deck_and_diff.csv", index=False)
        lead_table.to_csv(out_dir / "decisive_lead_timing.csv", index=False)
        move_delta_table.to_csv(out_dir / "move_type_material_deltas.csv", index=False)
        first_mover_table.to_csv(out_dir / "first_mover_win_rates.csv", index=False)
        first_points_table.to_csv(out_dir / "first_points_win_rates.csv", index=False)
        first_kings_table.to_csv(out_dir / "first_kings_win_rates.csv", index=False)

        # Plots
        try:
            plot_logit_win_prob(df, out_dir / "win_prob_logit.png")
            deck_buckets = sorted(
                [b for b in df["deck_bucket"].dropna().unique()], key=str
            )
            plot_win_rate_by_diff(df, np.array(bins, dtype=float), out_dir / "win_rate.png")
            plot_flip_by_diff(df, np.array(bins, dtype=float), out_dir / "flip_rate.png")
            plot_lead_timing(lead_table, out_dir / "lead_timing.png")
            plot_move_deltas(move_delta_table, out_dir / "move_deltas.png")
            plot_first_action_win_rates(
                first_mover_table, "Win rate: first mover", out_dir / "first_mover.png"
            )
            plot_first_action_win_rates(
                first_points_table,
                "Win rate: first to play points",
                out_dir / "first_points.png",
            )
            plot_first_action_win_rates(
                first_kings_table,
                "Win rate: first to play a king",
                out_dir / "first_kings.png",
            )
            if deck_buckets:
                plot_logit_by_deck_bucket(
                    df, out_dir / "win_prob_by_deck.png", deck_buckets
                )
                plot_win_rate_by_deck_and_diff(
                    df,
                    np.array(bins, dtype=float),
                    out_dir / "win_rate_by_deck.png",
                    deck_legend,
                )
                plot_flip_by_deck_and_diff(
                    df,
                    np.array(bins, dtype=float),
                    out_dir / "flip_rate_by_deck.png",
                    deck_legend,
                )
            ...
            plot_win_rate_heatmap(
                ...
                np.array(bins, dtype=float),
                deck_bins,
                out_dir / "win_rate_heatmap.png",
            )
            plot_flip_rate_heatmap(
                df,
                np.array(bins, dtype=float),
                deck_bins,
                out_dir / "flip_rate_heatmap.png",
            )
