...
- Implemented a point-aware scoring variant (point cards scaled by value; faces weighted K=2, Q=1.5, glasses=1.5; jacks excluded, offensive-card filter retained), added CLI flag `--point-aware-output-dir`, and generated full parallel artifacts in `artifacts-point-aware/` plus `takeaways-point-aware.md`.
 - Added a perspective-based model (`machine-learning/perspective_*`) that trains per-player win probability using only information that player knows. Iterated to a minimal feature set (own hand/points/faces, cards-needed, turn, opponent hand size only) and quantized whole-number weights (`artifacts-ml-perspective/weights_quantized.*`). Current simplified model: AUC ~0.68, Brier ~0.225; richer versions with full opponent info performed better but were simplified per request.
 - Added jack-stack preprocessing (`scripts/preprocess_jacks.py`) to reconstruct stolen-card stacks per game (tokens like `JJ10C`), emit `first_100k_gamestates_processed.csv`, and updated loaders/CLI/defaults to consume the processed CSV. Regenerated all baseline/point-aware artifacts and retrained the perspective model on the processed data (symmetric model removed).
